...
class ModelTree1(ModelTree):    
    root_alias = models.CharField('raíz', max_length=150, null=True, blank=True)
    last_alias = models.CharField('clave', max_length=150, null=True, blank=True)
    ...
    def _get_alias(self):
        if not self.parent:
            # return "%s__%s" % (self.root_alias, self.last_alias)
            # return "%s" % (self.root_alias) 
    ...
    def save(self, *args, **kwargs):
        ...
        self.root_alias = self._getRoot().root_alias
        return super().save(*args, **kwargs)
