...
class AdminSite1(admin.AdminSite):
    ...
    def __init__(self, name='admin'):   
        super().__init__(name)
        #-------------------------------------------
        # crear el superuser del website
        #------------------------------
        from django.contrib.auth.models import User, Group
        try:
            user_wsite = User.objects.get(username=settings.SITE_NAME)
            print_msg(" 2) Existe Superusuario SITE_NAME: %s" % user_wsite.username)
        except User.DoesNotExist:
            user_wsite = User.objects.create_superuser(username=settings.SITE_NAME,
    ...
    def register_models(self):        
        ...
        for fila in settings.MODELADMINS1:
...
class x_AdminSite1(admin.AdminSite):
    ...
    def __init__(self, name='admin'):
        ...
        #-----------------------
        from django.contrib.auth.models import User, Group
        from apps_auth.adjango.models import UserProp
        from apps_auth.companies.models import Company
        try:
        ...
        #----------------
        if settings.BIZ_ID:
            from apps_bpmn.business.models import Biz
            try:
    ...
    def menu_list(self, request):
        # import pdb; pdb.set_trace()
        from apps_auth.companies.models import Menu, MenuItem
        #-------------------------
        # Menu de la sesion
        #---------------------------
